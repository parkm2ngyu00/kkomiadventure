...
window.onload = function () {
	if (currentItem) {
		...
		document.getElementById("blackBox").textContent = currentItem.check[0];
		document.getElementById("greenBox").textContent = currentItem.check[1];
		document.getElementById("purpleBox").textContent = currentItem.check[2];
...
document
	...
	.addEventListener("click", function (event) {
		if (event.target.classList.contains("colorBox")) {
			// 클릭된 colorBox에 'enlarge' 클래스를 추가
			event.target.classList.add("enlarge");

			// 나머지 colorBox에 'enlarge' 클래스를 제거
			const otherBoxes = Array.from(
				document.getElementsByClassName("colorBox")
			).filter((box) => box !== event.target);
			otherBoxes.forEach((box) => box.classList.remove("enlarge"));
		}
	});
